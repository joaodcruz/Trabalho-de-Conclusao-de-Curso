...
// let map = L.map("mymap").setView([-23.325488, -51.201958], 16);
// let ourData = [];
                    ...
                    async function clickHandler() {
                        ...
                        try {
                            ...
                            if (mymap.style.display == "none") {
                                ...
                                let snapshot = await db.ref(`/${uid}/${nomeChart}/${nomeChart}/Dados`).orderByKey().startAt(TSdataInicio.toString()).endAt((TSdataInicio + 86400).toString()).once('value');
                                let dadosChart = snapshot.val();
                                ...
                                for (let key = TSdataInicio; key < (TSdataInicio + 86400); key++) {
                                    if (dadosChart[key] && dadosChart[key].Tempo && dadosChart[key].Fluxo !== undefined && dadosChart[key].Fluxo !== 0) {
                                        let fbFluxo = dadosChart[key].Fluxo;
                                        somaFluxo += fbFluxo;
                                        count1++;
                                    ...
                                    if((count1 + count0) == 180){
                                        ...
                                        mediaFluxo = somaFluxo / count1;
                                        chartFluxo.push(mediaFluxo);

                                        somaConsumo += (mediaFluxo * 3);
                                        chartConsumo.push(somaConsumo);
